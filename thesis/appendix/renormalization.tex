...
\begin{align}
\mu^{2\epsilon} &= 1 + 2\epsilon\ln\mu + \mathcal{O}(\epsilon^2), \\
...
In theories involving axial
currents, special care is required when extending the Dirac algebra to
$d$ dimensions because the definition of $\gamma^5$ is intrinsically
four-dimensional. In four spacetime dimensions the matrix $\gamma^5$
is defined as
\begin{equation}
\gamma^5 = i\gamma^0\gamma^1\gamma^2\gamma^3 ,
\end{equation}
but this definition cannot be straightforwardly generalized when the
dimension is analytically continued to $d=4-2\epsilon$. As discussed in
the original work by 't Hooft and Veltman on dimensional regularization
\cite{tHooftVeltman1972}, ambiguities can arise in loop integrals
involving $\gamma^5$ because not all four-dimensional gamma-matrix
identities remain valid in arbitrary dimensions. In practical
applications one often assumes that $\gamma^5$ can be defined in
$d$ dimensions such that it satisfies the usual relations
\[
\{\gamma^5,\gamma^\mu\}=0,
\qquad
(\gamma^5)^2=1,
\]
although these properties cannot be maintained consistently together
with all other Dirac algebra identities outside four dimensions.
Various prescriptions therefore exist for handling $\gamma^5$ in
dimensional regularization, typically involving modified
anticommutation relations or special treatment of Dirac traces.
However, in the present calculation no $\gamma^5$ matrices appear in
the loop integrals, and the dimensional continuation can therefore be
performed without introducing additional prescriptions. The remaining
Dirac identities used in the calculation, such as
...
\subsection*{Renormalization schemes: MS, $\overline{\mathbf{MS}}$, and on-shell}
