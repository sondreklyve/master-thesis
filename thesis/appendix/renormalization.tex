\chapter{Renormalization and Dimensional Regularization}
...
one-particle-irreducible (1PI) functions and Dyson resummation,
...
\subsection*{Ultraviolet divergences and dimensional regularization}
...
as \emph{ultraviolet} (UV) divergences.
...
Several regularization methods are commonly used in quantum field
theory.
In this work we employ dimensional regularization, which preserves
Lorentz invariance and gauge symmetry and is particularly convenient
in perturbative calculations.

The basic idea of dimensional regularization is to analytically
continue the number of spacetime dimensions away from four.
One replaces
\begin{equation}
...
d = 4 - 2\epsilon ,
\end{equation}
where $\epsilon$ is a small parameter and $\mu$ is an arbitrary mass
scale introduced to keep the integral dimensionally constant.
The original theory is recovered in the limit $\epsilon \to 0$.

Within dimensional regularization, ultraviolet divergences manifest
...
The factor $\mu^{2\epsilon}$ is introduced so that the integral
retains the correct mass dimension when the number of spacetime
dimensions is continued away from four.
The scale $\mu$ is arbitrary and is referred to as the
...
Ultraviolet divergences in dimensional regularization arise precisely
from these poles when the argument of the Gamma function approaches a
non-positive integer.

To make this explicit, consider the case $\alpha=1$.
In this case the numerator of \eqref{eq:Ialpha_gamma} contains the factor
\begin{equation}
\Gamma\!\left(1-\frac{d}{2}\right)
=
\Gamma(-1+\epsilon),
\end{equation}
which develops a simple pole as $\epsilon \to 0$.
...
Dimensional regularization thus converts ultraviolet divergences into
simple poles in the analytic continuation parameter $\epsilon$.

By contrast, power divergences are not represented in the same way in
dimensional regularization: integrals with no intrinsic scale (so-called
scaleless integrals), such as $\int d^d p\,(p^2)^\beta$, vanish identically.
For this reason dimensional regularization isolates the logarithmic UV
divergences---the ones relevant for renormalization---as poles in
$\epsilon$ in a particularly simple manner.

\subsection*{Renormalization and counterterms}
...
\subsection*{Perturbation theory, 1PI functions, and Dyson resummation}
...
In momentum space, the full propagator can be expressed in terms of
the self-energy through Dyson's equation
\cite{Dyson1949}.
Starting from the free propagator
\begin{equation}
G_0(p) = \frac{i}{p^2 - m_0^2 + i\epsilon},
\end{equation}
successive insertions of 1PI two-point diagrams generate a geometric
series.
Resumming this series yields
\begin{equation}
G(p)
=
...
\subsection*{Renormalization schemes: MS, $\overline{\mathrm{MS}}$, and on-shell}
...
In the minimal subtraction (MS) scheme, one removes only the pole terms
...
expressions and is widely used in quantum chromodynamics and other
gauge theories.

In minimal subtraction schemes, renormalized parameters depend
explicitly on the renormalization scale $\mu$ introduced in
