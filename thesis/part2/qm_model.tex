...
the zero-point energy of the quark field.
As discussed earlier, this term diverges because the momentum integral
extends to arbitrarily large momenta.
To render it well defined we evaluate the integral using dimensional
regularization, following the general framework outlined in
Appendix~\ref{app:renormalization}
\cite{PeskinSchroeder1995,Weinberg1995}.
...
\end{equation}
where $\mu$ is the mass scale introduced by dimensional regularization and
$M_q = g\bar\sigma$ is the constituent quark mass.
The factor $\mu^{3-d}$ ensures that the grand potential retains the
correct mass dimension away from three spatial dimensions.

Evaluating the momentum integral using dimensional regularization
(see Appendix~\ref{app:renormalization}) yields
\begin{equation}
\Omega_{\mathrm{vac}}
=
N_c N_f
...
\end{equation}
The pole $1/\epsilon$ signals a logarithmic ultraviolet divergence.

We now adopt the $\overline{\mathrm{MS}}$ renormalization scheme,
as defined in Appendix~\ref{app:renormalization}.
In this scheme one subtracts, together with the pole,
the associated constant terms $\ln(4\pi)-\gamma_E$.
It is convenient to introduce the renormalization scale
\begin{equation}
\Lambda^2 = 4\pi \mu^2 e^{-\gamma_E},
\end{equation}
so that the renormalized vacuum contribution takes the form
\begin{equation}
\Omega_{\mathrm{vac}}
=
N_c N_f
...
The renormalized mean-field grand potential therefore takes the form
\begin{equation}
\Omega(\bar\sigma;\mu_u,\mu_d)
=
U(\bar\sigma)
...
$\Omega_{\mathrm{med}}$ is the finite medium contribution.

\subsection{On-shell parameter fixing in vacuum}

The subtraction carried out above removes the ultraviolet divergence in
the vacuum contribution and yields a finite renormalized grand
potential $\Omega(\bar\sigma;\mu_u,\mu_d)$ at a chosen
$\overline{\mathrm{MS}}$ scale $\Lambda$.

In vacuum the medium contribution vanishes, so the effective potential
for constant fields reduces to
\begin{equation}
\Omega_{\mathrm{vac}}^{\mathrm{eff}}(\bar\sigma)
=
U(\bar\sigma)+\Omega_{\mathrm{vac}}^{\mathrm{ren}}(\bar\sigma),
\end{equation}
where $U(\bar\sigma)$ is given in
Eq.~\eqref{eq:classical_mesonic_potential} and
$\Omega_{\mathrm{vac}}^{\mathrm{ren}}$ in
Eq.~\eqref{eq:Omega_vac_ren_main}.

The vacuum contribution $\Omega_{\mathrm{vac}}^{\mathrm{ren}}$
depends on the constituent quark mass $M_q = g\bar\sigma$
and on the renormalization scale $\Lambda$, while the mesonic
potential contains the parameters $m^2$, $\lambda$, and $h$.
Consequently, the vacuum theory involves four independent parameters
\[
m^2,\qquad \lambda,\qquad h,\qquad g ,
\]
as well as the renormalization scale $\Lambda$ appearing in the
vacuum loop contribution.

Renormalization renders the grand potential finite but does not fix
these quantities.
To obtain physical predictions they must be determined by matching the
theory to experimentally known vacuum observables.
In practice this is achieved through an on-shell prescription
(see Appendix~\ref{app:renormalization}), where the parameters are
chosen such that the vacuum properties of the model reproduce known
physical quantities, such as the pion decay constant, meson masses,
and constituent quark mass \cite{PeskinSchroeder1995,WeinbergQFT}.

The physical vacuum is determined by the condition that the effective
potential is stationary with respect to variations of the condensate:
\begin{equation}
\left.
\frac{\partial \Omega_{\mathrm{vac}}^{\mathrm{eff}}(\bar\sigma)}
{\partial \bar\sigma}
...
In the linear realization of chiral symmetry the vacuum expectation
value of the scalar field is identified with the pion decay constant.
This follows from the matrix element of the axial current in the
linear sigma model \cite{GellMannLevy1960,SchaeferWambach2008}, which
relates the order parameter of spontaneous chiral symmetry breaking to
the pion decay constant. Consequently,
\begin{equation}
...
For the mesonic potential one obtains
\begin{equation}
...
Inserting the constituent quark mass $M_q=g\bar\sigma$ into
\eqref{eq:Omega_vac_ren_main} and differentiating with respect
to $\bar\sigma$ gives
\begin{equation}
\frac{\partial \Omega_{\mathrm{vac}}^{\mathrm{ren}}}{\partial \bar\sigma}
=
N_cN_f
\frac{g^4\bar\sigma^3}{4\pi^2}
\left[
\ln\!\left(\frac{\Lambda^2}{g^2\bar\sigma^2}\right)
+
1
\right].
\end{equation}

Evaluating the stationarity condition at $\bar\sigma=f_\pi$ yields
\begin{equation}
m^2 f_\pi
+
\lambda f_\pi^3
...
+
N_cN_f
\frac{g^4 f_\pi^3}{4\pi^2}
\left[
\ln\!\left(\frac{\Lambda^2}{g^2 f_\pi^2}\right)
+
1
\right]
=0 .
...
A convenient renormalization condition is to choose the scale
$\Lambda$ such that the vacuum-loop contribution to the 
stationarity condition vanishes at the
physical minimum. This stationarity-based scale choice is commonly
used in QM-model parameter fixing \cite{SchaeferWambach2008} and
fixes the renormalization scale to
\begin{equation}
\Lambda=\frac{g f_\pi}{\sqrt{e}} .
\label{eq:Lambda_fixed}
\end{equation}

With this stationarity-based scale choice, $\Lambda$ is fixed and the
vacuum minimum remains located at $\bar\sigma=f_\pi$.

The Yukawa interaction generates a constituent quark mass, which evaluated
in vacuum gives
\begin{equation}
M_q = g f_\pi .
\end{equation}
The constituent mass of light quarks inside hadrons is typically
$M_q \sim 300\,\mathrm{MeV}$ \cite{SchaeferWambach2008,Buballa2005}.
Using this value together with $f_\pi$ therefore determines the
Yukawa coupling.

To obtain explicit expressions for the curvature relations needed to
calculate the pion and sigma masses, we evaluate
the derivatives of the renormalized vacuum contribution.
It is convenient to express the effective potential in terms of the
chiral invariant
\begin{equation}
\rho = \bar\sigma^2 .
\end{equation}

Using Eq.~\eqref{eq:Omega_vac_ren_main}, the renormalized vacuum
contribution becomes
\begin{equation}
\Omega_{\mathrm{vac}}^{\mathrm{ren}}(\rho)
=
N_cN_f \frac{g^4\rho^2}{16\pi^2}
\left[
\ln\!\left(\frac{\Lambda^2}{g^2\rho}\right)+\frac{3}{2}
\right].
\label{eq:OmegaVac_rho}
\end{equation}

Differentiating with respect to $\rho$ yields
\begin{align}
\Omega_{\mathrm{vac}}^{\mathrm{ren}\,\prime}(\rho)
&=
N_cN_f \frac{g^4}{16\pi^2}
\left[
2\rho\left(\ln\!\left(\frac{\Lambda^2}{g^2\rho}\right)+\frac{3}{2}\right)
+\rho^2\left(-\frac{1}{\rho}\right)
\right]
\nonumber\\
&=
N_cN_f \frac{g^4\rho}{8\pi^2}
\left[
\ln\!\left(\frac{\Lambda^2}{g^2\rho}\right)+1
\right],
\label{eq:OmegaVac_prime}
\end{align}
and the second derivative becomes
\begin{equation}
\Omega_{\mathrm{vac}}^{\mathrm{ren}\,\prime\prime}(\rho)
=
N_cN_f \frac{g^4}{8\pi^2}
\ln\!\left(\frac{\Lambda^2}{g^2\rho}\right).
\label{eq:OmegaVac_dblprime}
\end{equation}

With the scale choice obtained from the vacuum stationarity condition,
Eq.~\eqref{eq:Lambda_fixed},
the derivatives evaluated at the vacuum expectation value become
\begin{equation}
\Omega_{\mathrm{vac}}^{\mathrm{ren}\,\prime}(f_\pi^2)=0,
\qquad
\Omega_{\mathrm{vac}}^{\mathrm{ren}\,\prime\prime}(f_\pi^2)
=
-\,N_cN_f \frac{g^4}{8\pi^2}.
\label{eq:OmegaVac_derivs_at_fpi}
\end{equation}

The pion mass is defined by the curvature of
$\Omega_{\mathrm{vac}}^{\mathrm{eff}}$ in a pion direction at the
vacuum minimum,
\begin{equation}
m_\pi^2
=
\left.
\frac{\partial^2 \Omega_{\mathrm{vac}}^{\mathrm{eff}}}
{\partial \pi_a^2}
\right|_{\bar\sigma=f_\pi,\;\bm{\pi}=0}
=
m^2+\lambda f_\pi^2+2\Omega_{\mathrm{vac}}^{\mathrm{ren}\,\prime}(f_\pi^2).
\label{eq:mpi_curvature}
\end{equation}
Using Eq.~\eqref{eq:OmegaVac_derivs_at_fpi}, this becomes
\begin{equation}
m_\pi^2 = m^2 + \lambda f_\pi^2.
\label{eq:mpi_OS_simple}
\end{equation}

Similarly, the sigma mass is defined by the curvature in the $\sigma$ direction,
\begin{equation}
m_\sigma^2
=
\left.
...
+2\Omega_{\mathrm{vac}}^{\mathrm{ren}\,\prime}(f_\pi^2)
+4f_\pi^2\Omega_{\mathrm{vac}}^{\mathrm{ren}\,\prime\prime}(f_\pi^2).
\label{eq:msigma_curvature}
\end{equation}
Substituting Eq.~\eqref{eq:OmegaVac_derivs_at_fpi} yields
\begin{equation}
m_\sigma^2 = m^2 + 3\lambda f_\pi^2
- N_cN_f\,\frac{g^4 f_\pi^2}{2\pi^2}.
\label{eq:msigma_OS_simple}
\end{equation}

Finally, we can determine $h$ by combining the stationarity condition
for the mesonic potential with the pion mass relation
\eqref{eq:mpi_OS_simple}. This gives
\begin{equation}
h = f_\pi m_\pi^2.
\end{equation}

The required physical inputs are
\begin{equation}
...
$m_\sigma$ and $M_q$ are standard phenomenological inputs in chiral
quark model studies \cite{SchaeferWambach2008,Buballa2005}. In
particular, $m_\sigma$ is model dependent because the scalar-isoscalar
channel is broad \cite{SchaeferWambach2008,Buballa2005}.
The relations derived above determine the model parameters
\begin{align}
g &= \frac{M_q}{f_\pi}, \\
h &= f_\pi m_\pi^2, \\
\lambda &= 
\frac{m_\sigma^2 - m_\pi^2 + N_c N_f \dfrac{g^4 f_\pi^2}{2\pi^2}}
{2 f_\pi^2}, \\
m^2 &= m_\pi^2 - \lambda f_\pi^2,
\end{align}
while the renormalization scale is fixed by the vacuum stationarity
condition,
\begin{equation}
\Lambda = \frac{g f_\pi}{\sqrt{e}} .
\end{equation}

These relations fully determine the renormalized parameter set of the
two-flavor Quark--Meson model in vacuum. Once fixed, this parameter set
can be used to compute thermodynamic observables and construct the
equation of state at finite chemical potential.
