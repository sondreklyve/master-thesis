...
propagators. The renormalized propagator denominator must therefore have
...
\end{align}
In the large-$N_c$ approximation, the relevant on-shell counterterms are
\begin{align}
...
\right].
\end{align}
The $\overline{\mathrm{MS}}$ counterterms subtract only the pole parts,
\begin{align}
...
by requiring that the vacuum condition is preserved when expressed in
terms of the running parameters. This leads to the condition
\begin{equation}
...
A useful consequence of the renormalization group equations is that the product
\begin{equation}
g^2_{\mathrm{MS}}(\Lambda)\,v^2_{\mathrm{MS}}(\Lambda)
=
g_0^2 v_0^2
...
\cite{Folkestad2018}. Using the standard result for the
vacuum integral, one finds
\begin{equation}
\Omega_{\mathrm{vac}}^{(1)}
=
...
be written directly in terms of the running parameters as
\begin{equation}
...
large-\(N_c\) approximation \cite{Folkestad2018}.
...
\(\Omega_{\mathrm{med}}=0\), and Eq.~\eqref{eq:qm_Omega_vac_ren}
gives the complete renormalized one-loop effective potential.
